...
trickshotTools()
{
    ...
    // Force UAV defaults ON, but perk stripping is delayed per player after spawn.
    // Starting the old loop immediately during the stock loadout window could make
    // Perk 2 display as None even for Stopping Power/Lightweight/Hardline.
    self.rainyForceUAVCanStripPerks = false;
    self thread rainyForceUAVInitialStripGrace();
...
createMenuHud()
{
    ...
    self.menuHudTitle = self createMenuTextFont("^5Rainy^7's ^7Mod ^5Menu ^7v^72", self.menuX + 5, self.menuY + 2, 1.06, "smallfixed", 9999);
...
menuTitleCursorLoop()
{
    ...
    for (;;)
    {
        if (!isDefined(self.menuHudTitle))
            return;
        self.menuHudTitle set_text("^5Rainy^7's ^7Mod ^5Menu ^7v^72 ^5_");
        wait 1.05;
        if (!isDefined(self.menuHudTitle))
            return;
        self.menuHudTitle set_text("^5Rainy^7's ^7Mod ^5Menu ^7v^72");
        wait 1.05;
    }
}
menuSelectBarPulse()
...
rainyForceUAVStripBlockerPerks()
{
    self _unsetPerk("specialty_coldblooded");
    self _unsetPerk("specialty_NINJA");
    self _unsetPerk("specialty_quieter");
...
rainyForceUAVInitialStripGrace()
{
    // Backup path for the first spawn: trickshotTools can start after the first
    // spawned_player notify has already fired, so do not rely only on waittill().
    self endon("disconnect");
    level endon("game_ended");
    level endon("rainy_shutdown");
    ...
    self.rainyForceUAVCanStripPerks = false;
    wait 2.0;
    self.rainyForceUAVCanStripPerks = true;

    if (isDefined(level.forceUAV) && level.forceUAV && isAlive(self))
        self rainyForceUAVStripBlockerPerks();
}
...
setForceUAVDvars(enabled)
{
    ...
    for (i = 0; i < level.players.size; i++)
    {
        ...
        if (enabled)
        {
            ...
            /*
                Cold-Blooded/Ninja still need to be stripped for Force UAV to work, but
                do NOT do it during the stock spawn/loadout window. Calling _unsetPerk()
                immediately at match start can corrupt the live perk HUD/loadout state and
                make Perk 2 display as None even when the class uses Stopping Power,
                Lightweight, or Hardline. The per-player spawn/grace watchers below open
                this gate only after the loadout has had time to settle.
            */
            if (isAlive(player) && isDefined(player.rainyForceUAVCanStripPerks) && player.rainyForceUAVCanStripPerks)
                player rainyForceUAVStripBlockerPerks();
        }
        else
        {
...
{
    // Mirrors rainyFovPersistLoop. Force UAV can default ON, but destructive
    // perk stripping is held back briefly after every spawn so it does not race
    // the stock class/loadout scripts and blank Perk 2.
    self endon("disconnect");
    level endon("game_ended");
    level endon("rainy_shutdown");
    ...
    for (;;)
    {
        ...
        // Close the gate immediately on each spawn/loadout refresh.
        self.rainyForceUAVCanStripPerks = false;
        ...
        // Give the stock loadout/perk HUD time to finish before _unsetPerk() is allowed.
        wait 1.5;
        self.rainyForceUAVCanStripPerks = true;
