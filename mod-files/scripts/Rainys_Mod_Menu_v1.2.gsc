...
createMenuHud()
{
    ...
    self.menuHudTitle = self createMenuTextFont("^5Rainy^7's ^7Mod ^5Menu ^7v^71.2", self.menuX + 5, self.menuY + 2, 1.06, "smallfixed", 9999);
...
menuTitleCursorLoop()
{
    ...
    for (;;)
    {
        if (!isDefined(self.menuHudTitle))
            return;
        self.menuHudTitle set_text("^5Rainy^7's ^7Mod ^5Menu ^7v^71.2 ^5_");
        wait 1.05;
        if (!isDefined(self.menuHudTitle))
            return;
        self.menuHudTitle set_text("^5Rainy^7's ^7Mod ^5Menu ^7v^71.2");
        wait 1.05;
    }
}
menuSelectBarPulse()
...
rainyRestartGame()
{
    ...
    // Full teardown right before the level actually reloads. closeMenuHud() above
    // only destroys the navigable menu's own HUD elements - the broadcast feed
    // (hitmarker/almost-hit/Trickshot Distance), the raised-message stack, and the
    // welcome banner each have their own separate HUD element sets that
    // closeMenuHud() never touches, and this function previously never set
    // level.rainyLevelShuttingDown or notified "rainy_shutdown" at all - meaning
    // every level endon("rainy_shutdown") watcher loop in this file kept running
    // uninterrupted right up until map_restart() pulled the level out from under
    // them. rainyTeardownMenu() is the exact same proven-correct full teardown the
    // natural game_ended path already uses before its own transition (see
    // rainyCleanupOnGameEnd) - its own comment spells out why this matters:
    // leftover client HUD elements are what crash the renderer re-init when
    // backing out of a match. map_restart() reruns main()/init() fresh afterward
    // (same as a genuine level load), so everything torn down here comes back up
    // cleanly on the next pass - this isn't losing any state permanently.
    level.rainyLevelShuttingDown = true;
    level notify("rainy_shutdown");
    self rainyTeardownMenu(true);
...
rainyInstantEndGame()
{
    ...
    // Same full-teardown fix as rainyRestartGame() above, for the same reason -
    // exitLevel() here fully exits the match back toward the lobby/main menu,
    // which is precisely the transition this file's own rainyTeardownMenu()
    // comment already warns leftover HUD elements crash the renderer during.
    // This path used to skip that teardown and the rainy_shutdown notify
    // entirely, unlike the natural game_ended flow, which is why this specific
    // button could crash on the way back to the main menu even when a normal
    // match ending didn't.
    level.rainyLevelShuttingDown = true;
    level notify("rainy_shutdown");
    self rainyTeardownMenu(true);
