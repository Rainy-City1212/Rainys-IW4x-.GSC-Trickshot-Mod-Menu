...
init()
{
    ...
    rainyEnsureAirdropCrateCollision();
    // Dedicated IW4x servers run with no connecting client ever satisfying
    // isHost() (the server process itself is the "host" - no player entity
    // is it), so every isHost()-gated feature in this menu is unreachable
    // there by default. This whitelist lets a server owner authorize
    // specific players by GUID via server config, independent of isHost().
    rainyInitOwnerWhitelist();
    // Single-owner lock for the whitelist: this menu was never built to
    // support more than one active menu holder at once (see
    // rainyTryClaimOwnership below), so this tracks which GUID currently
    // holds that single slot for the session. Empty = unclaimed.
    level.rainyClaimedOwnerGuid = "";
    level thread rainyDebugAirdropPallets();
    ...
    // Canswap Bind now defaults OFF for non-host players (see trickshotTools -
    // only the host gets it automatically), so the All Players page 2 toggle
    // display starts false too, matching the actual default state of every
    // non-host player's individual bind.
    level.rainyAllCanswap = false;
    // Admin Menu - Print Messages defaults ON. Set explicitly here for
    // clarity, matching this file's convention (see level.botsFrozen above) -
    // rainyShowRaisedMessage/rainyAddBroadcastFeedEntry also treat undefined
    // as ON defensively, so this isn't load-bearing, just documentation.
    level.rainyPrintMessagesEnabled = true;
...
// Builds the dedicated-server owner whitelist from a server dvar, e.g.
// (server config): set rainy_owner_guids "1100001abcde,1100001fghij"
// Comma-separated, no spaces. Leave unset/empty and behavior is unchanged
// (isHost() alone still governs everything on listen servers).
rainyInitOwnerWhitelist()
{
    level.rainyOwnerGuids = [];
    raw = getDvar("rainy_owner_guids");
    if (raw == "")
        return;
    parts = strTok(raw, ",");
    for (i = 0; i < parts.size; i++)
        level.rainyOwnerGuids[level.rainyOwnerGuids.size] = parts[i];
}
// Pure list-membership check - does NOT grant anything by itself, just
// answers "is this player's GUID on the dedicated-server whitelist". Used
// only by rainyTryClaimOwnership below.
rainyIsWhitelisted(player)
{
    if (!isDefined(player))
        return false;
    if (!isDefined(level.rainyOwnerGuids) || level.rainyOwnerGuids.size == 0)
        return false;
    playerGuid = player getGuid() + "";
    for (i = 0; i < level.rainyOwnerGuids.size; i++)
    {
        if (playerGuid == level.rainyOwnerGuids[i])
            return true;
    }
    return false;
}
// Decides ONCE, at connect time, whether this player becomes the menu's
// single active owner for this level session, and enforces that only one
// player can hold that slot at a time - this menu was never designed for
// two people running it concurrently (killcam prefs, FOV, quick-toggle
// binds, and the on-screen menu itself all gate off the self.rainyWasHost
// result of this call, not off isHost()/whitelist membership directly).
//
// - The real listen-server host always claims/overrides immediately. Only
//   one connected client can ever satisfy isHost() at once, so this can
//   never collide with a whitelist claim.
// - A non-host player claims ownership only if their GUID is whitelisted
//   (rainy_owner_guids) AND nobody currently holds the claim. If someone
//   else already holds it, this player is authorized-but-deferred: no menu
//   access for this connection. This check runs once at connect time, so a
//   deferred player is NOT automatically promoted when the current owner
//   later disconnects - they'd need to reconnect after that happens to
//   pick up the now-open claim.
rainyTryClaimOwnership(player)
{
    if (!isDefined(player))
        return false;
    player.rainyOwnGuid = player getGuid() + "";
    if (player isHost())
    {
        level.rainyClaimedOwnerGuid = player.rainyOwnGuid;
        return true;
    }
    if (!rainyIsWhitelisted(player))
        return false;
    if (isDefined(level.rainyClaimedOwnerGuid) && level.rainyClaimedOwnerGuid != "")
        return false;
    level.rainyClaimedOwnerGuid = player.rainyOwnGuid;
    return true;
}
trickshotGlobalEndGameMonitor()
{
    ...
    for (;;)
    {
        if (getDvar("ts_endgame") == "1")
        {
            ...
            // Full per-player HUD teardown before the level actually exits. Setting
            // the shutdown flag/notify above already stops every watcher loop, but
            // that only stops loops from building/updating HUD elements going
            // forward - it does nothing to destroy whatever elements a player
            // already had on screen (menu panel, broadcast feed, message stack,
            // welcome banner). Same leftover-HUD-elements-crash-the-renderer risk
            // rainyInstantEndGame()/rainyRestartGame() were just fixed for applies
            // here too, just via this separate F3 trigger instead of the menu button.
            if (isDefined(level.players))
            {
                for (i = 0; i < level.players.size; i++)
                {
                    if (isDefined(level.players[i]) && !level.players[i] isBot())
                        level.players[i] rainyTeardownMenu(true);
                }
            }
...
trickshotTools()
{
    ...
    // self.rainyWasRealHost is cached here (instead of calling isHost()
    // again later) because rainyCleanupOnDisconnect reads it AFTER the
    // disconnect notify has fired, and re-querying engine state at that
    // point is exactly what the caution note in that function warns against.
    self.rainyWasRealHost = self isHost();
    self.rainyWasHost = rainyTryClaimOwnership(self);
    // Canswap Bind now defaults ON for the host only, matching the same
    // host-default/host-grantable model as Save/Load Binds and UFO Mode
    // Bind - a non-host player no longer gets this bind automatically just
    // by connecting. The host can still grant it per-player (Player
    // Options individual submenu) or lobby-wide (All Players page 2),
    // both of which already start canswapBindMonitor() on their targets.
    self.canswapBound = self.rainyWasHost;
    if (self.canswapBound)
        self thread canswapBindMonitor();
    // Auto Refill Bind: display-only flag for the host (self.rainyWasHost is
    // always true here for host, so their individual submenu correctly shows
    // [ON]) - the host does NOT get autoRefillBindMonitor() threaded, because
    // they already have the exact same prone+Dpad-Up behavior built into
    // menuScrollUpListener via persistentHudMenuControls. Threading a second,
    // independent monitor on the same input for the host would fire
    // toggleAutoRefillAmmo() twice per press (once from each listener) and
    // cancel itself out. rainyToggleClientAutoRefillBind() only ever threads
    // autoRefillBindMonitor() for a confirmed non-host target, so this can
    // never happen.
    self.rainyAutoRefillBindEnabled = self.rainyWasHost;
    ...
    // TS Aimbot / Spawn Trickshot Platform quick-toggle binds stay host-only
    // (the host is the only one with menu access to those features in the
    // first place), so each monitor still self-gates internally rather than
    // being wrapped here.
    self thread tsAimbotBindMonitor();
    self thread spawnPlatformBindMonitor();
    // UFO Mode Bind now follows the same host-default/host-grantable model as
    // Save/Load Binds below: the host has it automatically, and a non-host
    // player only gets it if the host turns it on for them via that player's
    // individual submenu (rainyToggleClientUfoBind). ufoBindMonitor() no
    // longer self-gates on isHost() internally - see that function.
    self.rainyUfoBindEnabled = self.rainyWasHost;
    if (self.rainyUfoBindEnabled)
    ...
    // Save/Load Binds default to host-only - a non-host player no longer
    // inherits these binds automatically just by connecting. The host can
    // grant them to a specific player via that player's individual submenu
    // (rainyToggleClientSaveLoadBinds), which starts this same monitor on
    // that target when enabled.
    self.rainySaveLoadBindsEnabled = self.rainyWasHost;
    if (self.rainySaveLoadBindsEnabled)
    ...
    // Force UAV defaults ON lobby-wide. IW4x can replicate these compass-footstep
    // dvars as server info, so Force UAV no longer needs per-player client-dvar
    // persistence, spawn watchers, or any perk stripping. This keeps player loadouts
    // completely untouched while still making the constant radar state authoritative.
    if (!isDefined(level.forceUAVInitialized))
    {
        level.forceUAVInitialized = true;
        level.forceUAV = true;
        rainyApplyForceUAV(true);
    }
    self.forceUAV = level.forceUAV;
    // Wallbang Everything defaults ON lobby-wide, same model as canswapBound and
    // rainyTsPlatformBindOn above. Unlike those two, this one also drives real
    // server-wide penetration dvars, so - unlike the old version of this code - the
    // dvars are applied explicitly and immediately here rather than left to whichever
    // thread happens to tick the keep-alive loop first. This keeps self.wallbangOn and
    // the actual dvar state guaranteed consistent with each other from the first frame.
    ...
    wait 1;
    // Welcome banner is menu-owner-only (host, or the whitelisted player who
    // holds the single-owner claim on a dedicated server - see
    // rainyTryClaimOwnership). self.rainyWasHost is already resolved by this
    // point (set earlier in this same function), so this covers both listen
    // servers and dedicated servers with one check.
    if (self.rainyWasHost)
...
rainyShowRaisedMessage(msg)
{
    ...
    if (isDefined(level.rainyLevelShuttingDown) && level.rainyLevelShuttingDown)
        return;
    // Admin Menu - Print Messages master switch. Undefined is treated as ON
    // (default), matching this option's default-ON behavior before init()
    // has explicitly set it.
    if (isDefined(level.rainyPrintMessagesEnabled) && !level.rainyPrintMessagesEnabled)
        return;
...
persistentHudMenuControls()
{
    ...
    level endon("game_ended");
    level endon("rainy_shutdown");
    // Only the player who actually holds the single owner claim (real host,
    // or the one whitelisted player who claimed it - see
    // rainyTryClaimOwnership/rainy_owner_guids) may open/operate the
    // on-screen menu. Checking self.rainyWasHost here (the resolved claim
    // result) rather than re-deriving eligibility is what makes the
    // single-owner lock actually hold - a second whitelisted player who was
    // deferred at connect time must never slip through here. Non-owners
    // still keep their own keybinds (save/load position, and canswap when
    // the owner enables it for them) because those run in separate threads -
    // saveLoadBindMonitor and canswapBindMonitor - not in here. This
    // function governs ONLY menu navigation and open/close, so returning
    // early for non-owners removes their menu access without disabling any
    // of their binds.
    if (!self.rainyWasHost)
        return;
...
createMenuHud()
{
    ...
    self.menuHudTitle = self createMenuTextFont("^5Rainy^7's ^7Mod ^5Menu ^7v^72.1", self.menuX + 5, self.menuY + 2, 1.06, "smallfixed", 9999);
...
menuTitleCursorLoop()
{
    ...
    for (;;)
    {
        if (!isDefined(self.menuHudTitle))
            return;
        self.menuHudTitle set_text("^5Rainy^7's ^7Mod ^5Menu ^7v^72.1 ^5_");
        wait 1.05;
        if (!isDefined(self.menuHudTitle))
            return;
        self.menuHudTitle set_text("^5Rainy^7's ^7Mod ^5Menu ^7v^72.1");
        wait 1.05;
    }
}
menuSelectBarPulse()
...
getMenuPageLabel()
{
    ...
    if (self.menuPage == "adminmenu") return "ADMIN MENU";
...
getMenuMaxIndex()
{
    ...
    if (isSubStr(self.menuPage, "clientsub_"))
    {
        ...
        // real players. The 3 player-only items (Auto Refill Bind, Fast Last
        // FFA, Give TS Aimbot) each guard themselves at the function level
        // (rainyToggleClientAutoRefillBind/rainyClientFastLast/
...
updateMenuHud()
{
    ...
    if (self.menuPage == "main")
    {
        ...
        if (self.menuIndex == 7)
            self.menuHud7 set_text("   Admin Menu");
        else
            self.menuHud7 set_text("^5  Admin ^7Menu");
        return;
    }
    if (self.menuPage == "clients")
    {
    ...
    if (self.menuPage == "allplayers")
    {
        ...
        allRefillPlain = "OFF";
        if (isDefined(level.rainyAllAutoRefillBind) && level.rainyAllAutoRefillBind)
        {
            allRefillStatus = "^5ON";
            allRefillPlain = "ON";
        }
        if (self.menuIndex == 0)
            self.menuHud0 set_text("   All Auto Refill Bind [" + allRefillPlain + "]");
        else
            self.menuHud0 set_text("^5  All Auto Refill Bind ^7[" + allRefillStatus + "^7]");
        if (self.menuIndex == 1)
    ...
    if (self.menuPage == "allplayers2")
    {
        ...
        allUfoBindStatus = "^5OFF";
        allUfoBindPlain = "OFF";
        if (isDefined(level.rainyAllUfoBind) && level.rainyAllUfoBind)
        {
            allUfoBindStatus = "^5ON";
            allUfoBindPlain = "ON";
        }
        if (self.menuIndex == 3)
            self.menuHud3 set_text("   UFO Mode All [" + allUfoBindPlain + "]");
        else
            self.menuHud3 set_text("^7  UFO Mode All ^7[" + allUfoBindStatus + "^7]");

        allSaveLoadStatus = "^5OFF";
        allSaveLoadPlain = "OFF";
        if (isDefined(level.rainyAllSaveLoadBinds) && level.rainyAllSaveLoadBinds)
        {
            allSaveLoadStatus = "^5ON";
            allSaveLoadPlain = "ON";
        }
        if (self.menuIndex == 4)
            self.menuHud4 set_text("   Save/Load All [" + allSaveLoadPlain + "]");
        else
            self.menuHud4 set_text("^5  Save/Load All ^7[" + allSaveLoadStatus + "^7]");

        if (self.menuIndex == 5)
            self.menuHud5 set_text("   Page 1 <-");
        else
            self.menuHud5 set_text("^7  Page 1 ^5<-");
        return;
    }
    if (isSubStr(self.menuPage, "clientsub_"))
    {
        ...
        cRefillStatus = "^5OFF";
        cRefillPlain = "OFF";
        if (isDefined(cTarget) && isDefined(cTarget.rainyAutoRefillBindEnabled) && cTarget.rainyAutoRefillBindEnabled)
        {
            cRefillStatus = "^5ON";
            cRefillPlain = "ON";
        }
        if (self.menuIndex == 0)
            self.menuHud0 set_text("   Auto Refill Bind [" + cRefillPlain + "]");
        else
            self.menuHud0 set_text("^5  Auto Refill Bind ^7[" + cRefillStatus + "^7]");
    ...
    if (isSubStr(self.menuPage, "clientsub2_"))
    {
        ...
        cSaveLoadStatus = "^5OFF";
        cSaveLoadPlain = "OFF";
        if (isDefined(cTarget2) && isDefined(cTarget2.rainySaveLoadBindsEnabled) && cTarget2.rainySaveLoadBindsEnabled)
        {
            cSaveLoadStatus = "^5ON";
            cSaveLoadPlain = "ON";
        }
        if (self.menuIndex == 3)
            self.menuHud3 set_text("   Save/Load Binds [" + cSaveLoadPlain + "]");
        else
            self.menuHud3 set_text("^7  Save/Load Binds ^7[" + cSaveLoadStatus + "^7]");

        cUfoBindStatus = "^5OFF";
        cUfoBindPlain = "OFF";
        if (isDefined(cTarget2) && isDefined(cTarget2.rainyUfoBindEnabled) && cTarget2.rainyUfoBindEnabled)
        {
            cUfoBindStatus = "^5ON";
            cUfoBindPlain = "ON";
        }
        if (self.menuIndex == 4)
            self.menuHud4 set_text("   UFO Mode Bind [" + cUfoBindPlain + "]");
        else
            self.menuHud4 set_text("^5  UFO Mode Bind ^7[" + cUfoBindStatus + "^7]");
    ...
    if (self.menuPage == "adminmenu")
    {
        rbdLabel = "RBD Cycle Spawn";
        if (isDefined(level.rainyRbdCandidates) && level.rainyRbdCandidates.size > 0)
        {
            shownIdx = 0;
            if (isDefined(self.rainyRbdCycleIndex) && self.rainyRbdCycleIndex >= 0)
                shownIdx = self.rainyRbdCycleIndex;
            rbdLabel = "RBD Cycle Spawn [" + shownIdx + "/" + (level.rainyRbdCandidates.size - 1) + "]";
        }
        if (self.menuIndex == 0)
            self.menuHud0 set_text("   " + rbdLabel);
        else
            self.menuHud0 set_text("^5  " + rbdLabel);

        // Default ON: only show OFF once the flag has been explicitly set
        // false. Treating undefined as ON matches "toggled ON by default"
        ...
        if (self.menuIndex == 1)
            self.menuHud1 set_text("   Print Messages [" + printMsgPlain + "]");
        else
            self.menuHud1 set_text("^7  Print Messages ^7[" + printMsgStatus + "^7]");
        return;
    }
    if (self.menuPage == "bots")
    {
...
menuSelect()
{
    ...
    else if (self.menuPage == "adminmenu")
    {
        if (self.menuIndex == 0)
        {
            // rainyCycleRbdSpawn() already updates level.rainyRbdCandidates and
            // self.rainyRbdCycleIndex correctly on every press - the counter label
            // itself just doesn't repaint on its own. Every other place in this
            // menu that changes a row's on-screen text only actually redraws it via
            // a later scroll/page-change call to updateMenuHud() (see
            // menuScrollUpListener/menuScrollDownListener above); this page has
            // exactly one row, so there's no scroll to piggyback that refresh on
            // the way multi-item pages get it for free. Forcing it here makes the
            // "[x/y]" counter and the just-spawned index show correctly the moment
            // you press it, not only after you scroll away and back.
            self rainyCycleRbdSpawn();
            self updateMenuHud();
        }
        else if (self.menuIndex == 1)
        {
            self rainyTogglePrintMessages();
            self updateMenuHud();
        }
    }
    ...
    {
        if (self.menuIndex == 0)
            self thread rainyToggleAllAutoRefillBind();
        else if (self.menuIndex == 1)
    ...
    if (self.menuPage == "allplayers2")
    {
        if (self.menuIndex == 0)
            ...
            self thread rainyToggleAllUfoBind();
        else if (self.menuIndex == 4)
            self thread rainyToggleAllSaveLoadBinds();
        else if (self.menuIndex == 5)
        {
            self.menuPage = "allplayers";
            self.menuIndex = 9;
    ...
    {
        if (self.menuIndex == 0)
            self thread rainyToggleClientAutoRefillBind();
        else if (self.menuIndex == 1)
    ...
    if (isSubStr(self.menuPage, "clientsub2_"))
    {
        if (self.menuIndex == 0)
            ...
            self thread rainyToggleClientSaveLoadBinds();
        else if (self.menuIndex == 4)
            self thread rainyToggleClientUfoBind();
        else if (self.menuIndex == 5)
        {
...
allowClassChangeAlways()
{
    ...
    // Wait for a real, already-settled pers["class"] value before establishing
    // our baseline. Seeding lastClass from "" (the old behavior, if pers["class"]
    // wasn't assigned yet at the instant this thread started) let the very
    // FIRST class assignment - the one that happens automatically as part of a
    // normal spawn, not a deliberate reclass - look like a "change" to this
    // watcher. That fired our own manual giveLoadout() call racing the engine's
    // own native spawn-time loadout grant for that same class, and the race was
    // stripping perk slot 2 specifically right after it was set - reproducibly,
    // regardless of which perk occupied that slot (Cold-Blooded, Stopping Power
    // Pro, etc.), since it's a timing race and not tied to any specific perk id.
    // Waiting here for a settled value means this loop only ever reacts to
    // genuine, deliberate mid-life class changes - its actual intended purpose.
    while (!isDefined(self.pers["class"]) || self.pers["class"] == "")
        wait 0.1;
    ...
    for (;;)
    {
        ...
        if (currentClass != lastClass && currentClass != "")
        {
            ...
            if (isAlive(self))
            {
                ...
                if (!isDefined(playerTeam) || playerTeam == "")
                    playerTeam = "axis";
                // setClass() must run before giveLoadout() - this is the standard
                // paired call for applying a class change without dying (confirmed
                // against community MW2/IW4x "change class anytime" implementations).
                // setClass() resolves/caches which perks belong to this class slot;
                // giveLoadout() then reads that resolved state to actually grant
                // them. Calling giveLoadout() alone, as before, meant it could read
                // stale/partially-resolved class data - which slot ends up wrong
                // from that isn't tied to any specific perk, just whichever slot's
                // data wasn't freshly resolved. That's why it was always slot 2,
                // regardless of which perk (Cold-Blooded, Stopping Power Pro, etc.)
                // was actually assigned there.
                self maps\mp\gametypes\_class::setClass(currentClass);
...
rainyCleanupOnDisconnect()
{
    ...
    // from a private match to the lobby/main menu. isHost()/getGuid() are avoided here
    // for the same reason - self.rainyWasRealHost/self.rainyOwnGuid were cached at
    // connect time specifically so this function never has to query engine state
    // post-disconnect.
    //
    // Release the single-owner claim if this player was holding it, so a newly
    // connecting whitelisted player (dedicated server case) can claim ownership
    // going forward. This is separate from the shutdown decision below - a
    // whitelisted owner leaving a dedicated server should free up their claim
    // WITHOUT tearing down the whole menu system, since the dedicated server
    // process (and match) keeps running without them.
    if (isDefined(self.rainyWasHost) && self.rainyWasHost && isDefined(level.rainyClaimedOwnerGuid) && isDefined(self.rainyOwnGuid) && level.rainyClaimedOwnerGuid == self.rainyOwnGuid)
        level.rainyClaimedOwnerGuid = "";
    // Important: a normal non-owner player leaving should NOT broadcast rainy_shutdown.
    // That notify is level-wide and stops the host menu/broadcast loops for everyone.
    // Only the REAL host disconnecting (or an actual game-ending state) is treated as
    // the whole Rainy menu system shutting down - a listen server's match effectively
    // ends when its host leaves anyway. A whitelisted (non-host) owner leaving a
    // dedicated server does NOT trigger this - see the claim release above instead.
    if ((isDefined(self.rainyWasRealHost) && self.rainyWasRealHost) || rainyGameIsEnding())
    {
        level.rainyLevelShuttingDown = true;
        level notify("rainy_shutdown");
    }
...
rainyFreezeAllPlayers()
{
    ...
    for (i = 0; i < level.players.size; i++)
    {
        p = level.players[i];
        if (!isDefined(p)) continue;
        // Excludes the menu owner (real host, or the whitelisted dedicated-
        // server owner who holds the single-owner claim - see
        // rainyTryClaimOwnership) rather than raw isHost(). Raw isHost()
        // would miss a dedicated-server owner entirely, since isHost() is
        // never true for anyone there - this menu entry would end up
        // freezing the very person who pressed it.
        if (isDefined(p.rainyWasHost) && p.rainyWasHost) continue;
        rainyFreezeEntity(p);
        count++;
    }
...
removeDeathBarriers()
{
    ...
    // "Leave the combat area" auto-kill (self suicide(), reached internally via
    // maps\mp\_utility::_suicide) is confirmed to bypass callbackPlayerDamage
    // entirely - a rainyPlayerDamageHook diagnostic never fired for these
    // deaths, so filtering by MOD_SUICIDE there (tried previously) can never
    // catch this; replaceFunc() on the function itself is the only mechanism
    // that actually intercepts it, so it's back to being called here every
    // time this function runs (i.e. every level/restart), matching the
    // originally-working behavior. The earlier attempt to only install this
    // once per process (a dvar guard, to dodge a suspected replaceFunc-vs-VM-
    // reload crash) is what caused the out-of-bounds/suicide/respawn loop:
    // without a fresh install each level, _suicide() reverts to its real
    // stock behavior on level 2+. The original 0xC0000005 crash on leaving
    // the lobby needs to be re-tested in isolation with this restored - it's
    // possible the unrelated, already-fixed rainyTearDownUfo() velocity bug
    // (see that function) was a contributing or sole cause, not this call.
...
rainyConfigStringBudget()
{
    ...
    // build (r5046) hasn't been confirmed yet. Raise this only after confirming the real
    ...
    //
    // This is the MENU pool only (all set_text() calls from menu rows/toggles/pages).
    // The shot-feed (Trickshot Distance / Almost Hits / Hitmarkers) has its own separate
    // pool - see rainyFeedConfigStringBudget() - so heavy menu browsing can never crowd
    // those messages out, and a busy feed can never crowd out the menu. 420 + the feed's
    // 55 = 475 total, leaving 5 slots of pure slack under the conservative 480 ceiling.
    return 420;
}
rainyFeedConfigStringBudget()
{
    // Separate, reserved pool for the shot-feed messages only (Trickshot Distance,
    // Almost Hits, Hitmarkers - anything routed through rainyBroadcastMessage()). Kept
    // apart from the menu pool above so these vital messages always have the same
    // guaranteed amount of room every match, regardless of how many menu pages the
    // player has browsed - and so a long feed-heavy session can never eat into the
    // menu's own budget either.
    return 55;
}
rainyGuardConfigString(text)
{
    ...
    // MENU pool - see set_text(). Feed messages use rainyGuardFeedString() instead.
...
rainyConfigStringHasRoom(text)
{
    // Read-only PEEK version of rainyGuardConfigString() - answers "would this text
    // survive the menu budget?" without registering anything.
    if (!isDefined(text) || text == "")
        return true;
    if (!isDefined(level.rainyCfgSeen))
        return true;
    if (isDefined(level.rainyCfgSeen[text]))
        return true;
    return (level.rainyCfgCount < rainyConfigStringBudget());
}
rainyGuardFeedString(text)
{
    // Same mechanics as rainyGuardConfigString(), but against the dedicated FEED pool
    // (rainyFeedConfigStringBudget()) instead of the menu pool. Used only by
    // set_feed_text(), which the shot-feed HUD rows call instead of set_text().
    if (!isDefined(text) || text == "")
        return text;

    if (!isDefined(level.rainyCfgFeedSeen))
    {
        level.rainyCfgFeedSeen = [];
        level.rainyCfgFeedCount = 0;
    }

    if (isDefined(level.rainyCfgFeedSeen[text]))
        return text;

    if (level.rainyCfgFeedCount < rainyFeedConfigStringBudget())
    {
        level.rainyCfgFeedSeen[text] = true;
        level.rainyCfgFeedCount++;
        return text;
    }

    return "";
}
rainyFeedStringHasRoom(text)
{
    // Read-only PEEK version of rainyGuardFeedString() against the FEED pool. Called by
    // the shot-feed message builders (rainyAlmostHitCheck, rainyReportTrickshotHitAfterDamage)
    // BEFORE queueing anything, so that once the feed pool is exhausted a brand-new
    // distance/attacker/victim combination simply never fires - no queue entry, no blank
    // flash taking up one of the feed's visible rows for its hold+fade duration. Every
    // message that plays while there's room behaves exactly as it always has; this only
    // changes behavior in the (rare, budget-exhausted) case where it would have rendered
    // blank anyway.
    if (!isDefined(text) || text == "")
        return true;
    if (!isDefined(level.rainyCfgFeedSeen))
    ...
    if (isDefined(level.rainyCfgFeedSeen[text]))
        return true;
    return (level.rainyCfgFeedCount < rainyFeedConfigStringBudget());
}
rainyIsDigit(ch)
{
...
set_feed_text(text)
{
    // Identical to set_text() except it draws from the dedicated FEED budget
    // (rainyGuardFeedString) instead of the menu budget. Used only by the shot-feed
    // HUD rows (rainyRenderBroadcastSlots) so Trickshot Distance / Almost Hits /
    // Hitmarker text can never be crowded out by menu browsing, and vice versa.
    // Still runs through rainyCollapseRowColor for parity with set_text(), though feed
    // messages don't match the toggle-row pattern so this only forces element color to
    // white and returns the text unchanged - same as it always has.
    if (!isDefined(self) || !isDefined(text))
        return;
    text = self rainyCollapseRowColor(text);
    text = rainyGuardFeedString(text);
    self.text = text;
    self setText(text);
}
unfreezeAllBots()
...
toggleForceUAV()
{
    ...
    self.forceUAV = level.forceUAV;
    rainyApplyForceUAV(level.forceUAV);
...
rainyApplyForceUAV(enabled)
{
    if (enabled)
    {
        // IW4x Force-UAV fix: abuse the footstep compass system and publish the
        // values as server info so every client uses the same constant-radar state.
    ...
    {
        // Restore the stock MW2/IW4x values when Force UAV is toggled OFF.
...
rainyTearDownUfo()
{
    // Captured BEFORE self.ufoEnabled is touched below and BEFORE unlink() runs.
    // Do not switch this to reading self.ufoEnabled instead: toggleUFO() already
    // flips self.ufoEnabled to false itself before calling this function on its
    // OFF path, so checking that field in here would always read false for a
    // real UFO-off action too. self.ufoEntity is untouched by every call site
    // until this function's own cleanup at the bottom, so it's a reliable,
    // caller-order-independent signal of whether a UFO script_origin link
    // actually existed just now.
    wasLinkedToUfo = isDefined(self.ufoEntity);
    self.ufoEnabled = false;
    self unlink();
    ...
    wait 0.1;
    self enableweapons();
    // Only clear velocity if UFO was actually linked. This used to run
    // unconditionally for every caller, including rainyResetTransientPlayerSettings
    // (called the instant game_ended fires for every player, host included). A
    // trickshot that ends the match fires this mid-air, on a player who was never
    // in UFO mode at all - zeroing their velocity right as the stock engine's own
    // endgame control-freeze kicks in left them permanently suspended in place for
    // the rest of the victory screen instead of falling naturally like vanilla.
    if (wasLinkedToUfo)
        self SetVelocity((0, 0, 0));
    if (isDefined(self.ufoEntity))
    {
...
rainyAddBroadcastFeedEntry(msg)
{
    ...
    if (isDefined(level.rainyLevelShuttingDown) && level.rainyLevelShuttingDown)
        return;
    // Admin Menu - Print Messages master switch. Same gate as
    // rainyShowRaisedMessage - this is the other half of "any text the mod
    // menu prints" (Trickshot Distance / Almost Hits / Hitmarkers).
    if (isDefined(level.rainyPrintMessagesEnabled) && !level.rainyPrintMessagesEnabled)
        return;
...
rainyRenderBroadcastSlots()
{
    ...
    for (i = 0; i < maxMsgs; i++)
    {
        ...
        if (isDefined(self.rainyShotFeedMsgs) && isDefined(self.rainyShotFeedMsgs[i]))
        {
            ...
            hud set_feed_text(self.rainyShotFeedMsgs[i]);
            hud.alpha = alpha;
...
rainyAlmostHitCheck(weapon, shotId, shotTime)
{
    ...
    // Feed-budget guard rail: if this exact (shooter, target, meters) string would come
    // back blank anyway, don't fire it at all - no queue entry, no blank flash eating a
    // visible feed row for its hold+fade duration. Every message that has room behaves
    // exactly as before; this only changes anything once the feed pool is exhausted.
    if (!rainyFeedStringHasRoom(msg))
        return;
    rainyBroadcastMessage(msg);
}
...
rainyReportTrickshotHitAfterDamage(victim, shotMeters, attackerName, victimName)
{
    ...
    if (!isAlive(victim))
    {
        if (isDefined(self.trickshotDistanceOn) && self.trickshotDistanceOn)
        {
            ...
            // Same feed-budget guard rail as Almost Hits: skip firing entirely rather
            // than queue a blank flash once the reserved feed pool is exhausted.
            if (rainyFeedStringHasRoom(msg))
                rainyBroadcastMessage(msg);
        }
    }
...
/*
    Player Options individual submenu, Page 1 index 0: grants/revokes the
    Auto Refill Bind (Prone + Dpad Up) rather than directly flipping the
    target's ammo-refill state. This is the same bind the host already gets
    for free via menuScrollUpListener (prone + ts_menu_up), extracted into
    its own standalone monitor (autoRefillBindMonitor) so it can be handed to
    a non-host player without also handing them the host's menu-scroll
    notifyonplayercommand registrations - persistentHudMenuControls (and menu
    access itself) stays host/owner-only. Once granted, the target controls
    their OWN Auto Refill Ammo the same way the host does; this menu entry
    only ever toggles whether they HAVE that control, never the ammo state
    directly.
*/
rainyToggleClientAutoRefillBind()
{
    self endon("disconnect");
    target = self.clientSubTarget;
    if (!isDefined(target))
    ...
    if (isDefined(target.rainyWasHost) && target.rainyWasHost)
    {
        self thread rainyShowRaisedMessage("^5Host always has Auto Refill Bind");
        return;
    }
    if (!isDefined(target.rainyAutoRefillBindEnabled) || !target.rainyAutoRefillBindEnabled)
    {
        target.rainyAutoRefillBindEnabled = true;
        target thread autoRefillBindMonitor();
        self thread rainyShowRaisedMessage("^5Auto Refill Bind ^7[^5ON^7] - " + target.name + " (Prone + Dpad Up)");
    }
    else
    {
        target.rainyAutoRefillBindEnabled = false;
        target notify("stop_autorefill_bind");
        self thread rainyShowRaisedMessage("^5Auto Refill Bind ^7[^5OFF^7] - " + target.name);
    }
}
rainyClientFastLast()
{
...
rainyToggleClientSaveLoadBinds()
{
    self endon("disconnect");
    target = self.clientSubTarget;
    if (!isDefined(target))
    ...
    if (isDefined(target.rainyWasHost) && target.rainyWasHost)
    {
        self thread rainyShowRaisedMessage("^5Host always has Save/Load Binds");
        return;
    }
    if (!isDefined(target.rainySaveLoadBindsEnabled) || !target.rainySaveLoadBindsEnabled)
    {
        target.rainySaveLoadBindsEnabled = true;
        target thread saveLoadBindMonitor();
        self thread rainyShowRaisedMessage("^5Save/Load Binds ^7[^5ON^7] - " + target.name + " (Prone/Crouch + Dpad Down)");
    }
    else
    {
        target.rainySaveLoadBindsEnabled = false;
        target notify("stop_saveload_bind");
        self thread rainyShowRaisedMessage("^5Save/Load Binds ^7[^5OFF^7] - " + target.name);
    }
}
rainyToggleClientUfoBind()
{
    self endon("disconnect");
    target = self.clientSubTarget;
    if (!isDefined(target))
    ...
    if (isDefined(target.rainyWasHost) && target.rainyWasHost)
    {
        self thread rainyShowRaisedMessage("^5Host always has UFO Mode Bind");
        return;
    }
    if (!isDefined(target.rainyUfoBindEnabled) || !target.rainyUfoBindEnabled)
    {
        target.rainyUfoBindEnabled = true;
        target thread ufoBindMonitor();
        self thread rainyShowRaisedMessage("^5UFO Mode Bind ^7[^5ON^7] - " + target.name + " (Aim + Standing + Dpad Down)");
    }
    else
    {
        target.rainyUfoBindEnabled = false;
        target notify("stop_ufo_bind");
        // If the player was mid-flight when the bind was revoked, force UFO
        // off cleanly (same teardown toggleUFO() itself uses) rather than
        // leaving them stuck floating with no bind left to turn it off.
        if (isDefined(target.ufoEnabled) && target.ufoEnabled)
            target rainyTearDownUfo();
        self thread rainyShowRaisedMessage("^5UFO Mode Bind ^7[^5OFF^7] - " + target.name);
    }
}
...
/*
    All Players page 1 - Auto Refill Bind: same effect as the individual
    per-player Auto Refill Bind toggle (rainyToggleClientAutoRefillBind),
    applied to every connected real player at once - bots and the host are
    both excluded (host already has this bind built in and can't be
    granted/revoked through this system - see autoRefillBindMonitor's
    comment). level.rainyAllAutoRefillBind tracks the group toggle state
    shown in the menu, since this isn't any single entity's flag.
*/
rainyToggleAllAutoRefillBind()
{
    if (!isDefined(level.players))
    {
        self thread rainyShowRaisedMessage("^5No players found");
    ...
    if (!isDefined(level.rainyAllAutoRefillBind))
        level.rainyAllAutoRefillBind = false;
    level.rainyAllAutoRefillBind = !level.rainyAllAutoRefillBind;
    count = 0;
    for (i = 0; i < level.players.size; i++)
    {
        p = level.players[i];
        ...
        if (rainyIsBot(p)) continue;
        if (isDefined(p.rainyWasHost) && p.rainyWasHost) continue;
        if (level.rainyAllAutoRefillBind)
        {
            // Guarded the same way rainyToggleAllCanswap already does it -
            // without this check, a player whose bind was already enabled
            // (an earlier individual grant) would get a second
            // autoRefillBindMonitor() thread stacked on the still-running
            // first one, double-firing toggleAutoRefillAmmo() per press.
            if (!isDefined(p.rainyAutoRefillBindEnabled) || !p.rainyAutoRefillBindEnabled)
                p thread autoRefillBindMonitor();
            p.rainyAutoRefillBindEnabled = true;
        }
        else
        {
            p.rainyAutoRefillBindEnabled = false;
            p notify("stop_autorefill_bind");
        }
        count++;
    }
    if (level.rainyAllAutoRefillBind)
        self thread rainyShowRaisedMessage("^5Auto Refill Bind ^7[^5ON^7] - " + count + " players (Prone + Dpad Up)");
    else
        self thread rainyShowRaisedMessage("^5Auto Refill Bind ^7[^5OFF^7] - " + count + " players");
}
/*
...
rainyToggleAllCanswap()
{
    ...
    for (i = 0; i < level.players.size; i++)
    {
        ...
        {
            // Guarded the same way the per-client toggle already does it
            // (rainyToggleClientCanswap) - without this check, a player whose
            // canswapBound was already true (the default at connection, or from
            // an earlier individual toggle) would get a SECOND
            // canswapBindMonitor() thread stacked on top of the still-running
            // first one, since nothing here ever fired stop_canswap_bind to
            // kill the existing one first. Two live monitors both listening
            // for the same rainy_canswap_btn notify means one Canswap Bind
            // press could fire doCanswap() twice in a row for that player.
            if (!isDefined(p.canswapBound) || !p.canswapBound)
...
/*
    All Players page 2 - UFO Mode Bind: same effect as the individual
    per-player UFO Mode Bind toggle (rainyToggleClientUfoBind), applied to
    every connected real player at once - bots and the host are both
    excluded (host already has this bind built in and can't be
    granted/revoked through this system). level.rainyAllUfoBind tracks the
    group toggle state shown in the menu.
*/
rainyToggleAllUfoBind()
{
    if (!isDefined(level.players))
    {
        self thread rainyShowRaisedMessage("^5No players found");
    ...
    if (!isDefined(level.rainyAllUfoBind))
        level.rainyAllUfoBind = false;
    level.rainyAllUfoBind = !level.rainyAllUfoBind;
    count = 0;
    for (i = 0; i < level.players.size; i++)
    {
        p = level.players[i];
        ...
        if (rainyIsBot(p)) continue;
        if (isDefined(p.rainyWasHost) && p.rainyWasHost) continue;
        if (level.rainyAllUfoBind)
        {
            // Guarded the same way rainyToggleAllCanswap already does it -
            // avoids stacking a second ufoBindMonitor() thread on a player
            // who already has the bind (default state or an earlier
            // individual grant), which would double-fire toggleUFO() per press.
            if (!isDefined(p.rainyUfoBindEnabled) || !p.rainyUfoBindEnabled)
                p thread ufoBindMonitor();
            p.rainyUfoBindEnabled = true;
        }
        else
        {
            p.rainyUfoBindEnabled = false;
            p notify("stop_ufo_bind");
            // Same mid-flight safety net as the individual toggle - force
            // UFO off cleanly rather than stranding the player floating
            // with no bind left to turn it off.
            if (isDefined(p.ufoEnabled) && p.ufoEnabled)
                p rainyTearDownUfo();
        }
        count++;
    }
    if (level.rainyAllUfoBind)
        self thread rainyShowRaisedMessage("^5UFO Mode Bind ^7[^5ON^7] - " + count + " players (Aim + Standing + Dpad Down)");
    else
        self thread rainyShowRaisedMessage("^5UFO Mode Bind ^7[^5OFF^7] - " + count + " players");
}
/*
    All Players page 2 - Save/Load Binds: same effect as the individual
    per-player Save/Load Binds toggle (rainyToggleClientSaveLoadBinds),
    applied to every connected real player at once - bots and the host are
    both excluded (host already has this bind built in and can't be
    granted/revoked through this system). level.rainyAllSaveLoadBinds tracks
    the group toggle state shown in the menu.
*/
rainyToggleAllSaveLoadBinds()
{
    if (!isDefined(level.players))
    {
        self thread rainyShowRaisedMessage("^5No players found");
    ...
    if (!isDefined(level.rainyAllSaveLoadBinds))
        level.rainyAllSaveLoadBinds = false;
    level.rainyAllSaveLoadBinds = !level.rainyAllSaveLoadBinds;
    count = 0;
    for (i = 0; i < level.players.size; i++)
    {
        p = level.players[i];
        ...
        if (rainyIsBot(p)) continue;
        if (isDefined(p.rainyWasHost) && p.rainyWasHost) continue;
        if (level.rainyAllSaveLoadBinds)
        {
            // Guarded the same way rainyToggleAllCanswap already does it -
            // avoids stacking a second saveLoadBindMonitor() thread on a
            // player who already has the bind.
            if (!isDefined(p.rainySaveLoadBindsEnabled) || !p.rainySaveLoadBindsEnabled)
                p thread saveLoadBindMonitor();
            p.rainySaveLoadBindsEnabled = true;
        }
        else
        {
            p.rainySaveLoadBindsEnabled = false;
            p notify("stop_saveload_bind");
        }
        count++;
    }
    if (level.rainyAllSaveLoadBinds)
        self thread rainyShowRaisedMessage("^5Save/Load Binds ^7[^5ON^7] - " + count + " players (Prone/Crouch + Dpad Down)");
    else
        self thread rainyShowRaisedMessage("^5Save/Load Binds ^7[^5OFF^7] - " + count + " players");
}
/*
...
rainyForgeModeLoop()
{
    ...
    while (true)
    {
        ...
        if (isDefined(trace["entity"]))
        {
            ...
            ent = trace["entity"];
            if (!self.menuOpen && self adsButtonPressed())
            {
                while (!self.menuOpen && self adsButtonPressed() && isDefined(ent))
                {
                    /*
                        Do not restart a 0.5-second moveTo every 0.01 seconds. On
                        settled/physics-controlled entities (especially stock care
                        packages), direct .origin writes can be ignored; that left the
                        repeatedly restarted moveTo as the only movement command, so the
                        object appeared completely frozen even though ADS was detected.

                        setOrigin is an immediate engine-side teleport and is the normal
                        Forge pattern for holding an entity at a point in front of the
                        player. Re-applying it at the same rate as this loop keeps both
                        ordinary script models and landed care packages responsive.
                    */
                    // setOrigin is a player-only method in this build - every other
                    // SetOrigin call in this file targets self/bot/target, never a
                    // non-player entity like a crate. Calling it on ent here was
                    // dispatching a player-only native against a script_model handle,
                    // which is the likely source of the fatal crash. A direct .origin
                    // write is the same pattern this file already uses everywhere else
                    // to move non-player entities (e.g. the care package keep-alive
                    // loop writes crate fields directly, never via a method call).
                    forgeHoldOrigin = self getTagOrigin("j_head") + anglesToForward(self getPlayerAngles()) * 200;
                    ent.origin = forgeHoldOrigin;
                    wait .05;
                }
            }
...
/*
    Auto Refill Bind - Prone + Dpad Up (+actionslot 1). Same physical button
    as Canswap Bind above, no collision since Canswap requires standing and
    this requires prone - mutually exclusive stances, exactly like the
    existing three-way share on +actionslot 2 (menu scroll-down / UFO /
    Save-Load) documented below.

    The host gets this same behavior for free through menuScrollUpListener
    (which also owns +actionslot 1 via ts_menu_up, for menu scroll-up and
    this same prone-toggle in one combined listener) - this standalone
    monitor is only ever started for a non-host player the host has granted
    it to via rainyToggleClientAutoRefillBind, using its own independent
    "rainy_autorefill_btn" notify so it never depends on the host-only
    ts_menu_up registrations inside persistentHudMenuControls.
*/
autoRefillBindMonitor()
{
    self endon("disconnect");
    self endon("stop_autorefill_bind");
    level endon("game_ended");
    level endon("rainy_shutdown");
    self notifyOnPlayerCommand("rainy_autorefill_btn", "+actionslot 1");
    for (;;)
    {
        self waittill("rainy_autorefill_btn");
        if (isDefined(self.menuOpen) && self.menuOpen)
            continue;
        if (self GetStance() != "prone")
            continue;
...
/*
    Quick-toggle binds for UFO Mode, TS Aimbot, and Spawn Trickshot Platform.
    Each just calls the exact same function the menu item already calls
    (toggleUFO/toggleTsAimbot/spawnPlatformGrid) - the bind is purely an
    alternate trigger, not a separate code path, so there's nothing new to
    keep in sync if those functions ever change.

    TS Aimbot and Spawn Trickshot Platform stay strictly host-only (self-gated
    inside each of those two monitors). UFO Mode Bind is host-default but
    host-grantable per non-host player (see rainyToggleClientUfoBind) -
    ufoBindMonitor() itself no longer checks isHost() and is simply only
    started for whichever players currently have it enabled.

    Bind/stance map for all three:
      - UFO Mode:               D-pad down (+actionslot 2), aim + standing only
      - TS Aimbot:               D-pad up   (+actionslot 1), crouch only (host-only)
      - Spawn Trickshot Platform: D-pad left (+actionslot 3), crouch only (host-only)
    ...
    standing-only window; it doesn't change this non-overlap with the other
    two. This still holds for a non-host player granted UFO and/or Save/Load
    Binds - menu scroll-down never applies to them regardless (menu access
    itself stays host/owner-only), and UFO's standing-only window still never
    overlaps Save/Load's prone/crouch-only window on the same player.
...
{
    self endon("disconnect");
    self endon("stop_ufo_bind");
    level endon("game_ended");
    level endon("rainy_shutdown");
    // No isHost() gate here anymore - this thread is only ever started when
    // self.rainyUfoBindEnabled is true (host by default, or a non-host
    // player the host has granted it to via rainyToggleClientUfoBind), so
    // gating again here would be redundant, and for a granted non-host
    // player it would incorrectly block the very access just granted.
...
rainyDebugAirdropPallets()
{
    /*
        Diagnostic scan, not a fix. Whatever format the source .ff was in
        (plain IWffu100 or an encrypted IW4x-native zone) is irrelevant here
        - by the time this runs, the client has already fully decrypted and
        loaded the map, so every entity on it is just sitting in memory,
        readable the normal way. This sidesteps needing to inspect the
        compiled file at all.

        Finds every entity tagged script_gameobjectname "airdrop_pallet",
        plus the known fixed-name fallbacks seen on ported maps so far, then
        EMPIRICALLY tests each one: clone its brush onto a throwaway test
        crate and fire a trace straight through it. A trace that passes
        through means that "collision" is a dead placeholder (common on
        maps where a porting tool inserted the entity just so mapscript
        logic expecting it wouldn't error, without ever giving it a real
        brush). A trace that hits means it's genuinely solid and usable.

        Results print to console (println, not a HUD message) since this is
        meant to be a scrollable report, not something that has to fit in a
        HUD message stack.
    */
    println("^3===== Rainy Debug: Airdrop Pallet / Care Package Collision Scan =====");

    candidates = [];

    pallets = getEntArray("airdrop_pallet", "script_gameobjectname");
    if (isDefined(pallets))
    {
        for (i = 0; i < pallets.size; i++)
            candidates[candidates.size] = pallets[i];
    }

    // Known fixed targetnames from maps checked so far that weren't always
    // tagged with the script_gameobjectname key. Extend this list as new
    // maps turn up other naming conventions.
    fixedNames = [];
    fixedNames[0] = "iw3x_entity_carepackage";
    fixedNames[1] = "iw3x_entity_airdroppallet";
    fixedNames[2] = "airdrop_crate_collision";
    fixedNames[3] = "care_package_collision";

    for (i = 0; i < fixedNames.size; i++)
    {
        ent = getEnt(fixedNames[i], "targetname");
        if (!isDefined(ent))
            continue;
        ...
        if (!alreadyListed)
            candidates[candidates.size] = ent;
    }

    if (candidates.size == 0)
    {
        println("^1[Rainy Debug] No airdrop_pallet-tagged or known-fallback-named entities exist on this map at all.");
        println("^1[Rainy Debug] Nothing to scavenge here - a real recompile (new brush via iw3x-port/ZoneBuilder) is the only fix.");
        println("^3===== End Scan =====");
        return;
    }

    println("^3[Rainy Debug] Found " + candidates.size + " candidate entity(ies). Testing each for real collision...");
    ...
    for (i = 0; i < candidates.size; i++)
    {
        ...
        org = ent.origin;
        println("^3[Rainy Debug] --- Candidate " + i + ": targetname=" + tname + " origin=(" + org[0] + ", " + org[1] + ", " + org[2] + ") ---");
        rainyDebugTestCandidateCollision(ent);
    }

    println("^3===== End Scan =====");
}

rainyDebugTestCandidateCollision(candidateEnt)
{
    // Spawned well above the candidate's own origin so the test crate never
    // clips into unrelated world geometry - CloneBrushModelToScriptModel
    // re-parents the collision onto wherever THIS crate sits, not the
    // original brush's fixed map location (confirmed by the existing
    // platform-spawn code already behaving this way), so the test crate's
    // exact position doesn't need to match the source brush's position.
    testPos = candidateEnt.origin + (0, 0, 500);
    crate = spawn("script_model", testPos);
    crate setModel("com_plasticcase_friendly");
    crate Solid();
    crate CloneBrushModelToScriptModel(candidateEnt);

    traceStart = testPos + (0, 0, 40);
    traceEnd = testPos - (0, 0, 40);
    trace = bulletTrace(traceStart, traceEnd, false, undefined);

    if (isDefined(trace) && isDefined(trace["fraction"]) && trace["fraction"] < 1)
        println("^2[Rainy Debug]     RESULT: SOLID - trace hit at fraction " + trace["fraction"] + ". Real, usable collision.");
    else
        println("^1[Rainy Debug]     RESULT: NOT SOLID - trace passed straight through. Dead placeholder.");

    crate delete();
}
rainySetPlatformCollisionDonor(ent, label)
{
    if (!isDefined(ent))
        return false;

    level.airDropCrateCollision = ent;
    level.rainyPlatformCollisionResolved = true;

    if (!isDefined(level.rainyPlatformCollisionSource) || level.rainyPlatformCollisionSource != label)
    {
        println("^2[Rainy Platforms] Using platform collision donor from " + label + ".");
        level.rainyPlatformCollisionSource = label;
    }

    return true;
}

rainyTryExplicitPlatformCollision()
{
    /*
        Drop-in .ents hook.

        Preferred .ents patch for a known-good brushmodel:
        1) Leave the original brush entity alone.
        2) ADD a duplicate script_brushmodel entity using the same compiled model number.
        3) Give the duplicate this targetname:

           "targetname" "rainy_platform_collision"

        Example for mp_luna after candidate #0 was proven solid:

           {
           "classname" "script_brushmodel"
           "targetname" "rainy_platform_collision"
           "script_gameobjectname" "rainy_platform_collision"
           "origin" "2639 1963 27"
           "model" "*1"
           }

        A duplicate alias entity is more reliable than editing arbitrary custom
        fields onto the map's existing gameplay brush. It also avoids breaking
        the map's own targetname/script logic.
    */

    ent = getEnt("rainy_platform_collision", "targetname");
    if (isDefined(ent))
        return rainySetPlatformCollisionDonor(ent, "targetname rainy_platform_collision");

    ent = getEnt("rainy_care_package_collision", "targetname");
    if (isDefined(ent))
        return rainySetPlatformCollisionDonor(ent, "targetname rainy_care_package_collision");

    ents = getEntArray("rainy_platform_collision", "script_gameobjectname");
    if (isDefined(ents) && ents.size > 0)
        return rainySetPlatformCollisionDonor(ents[0], "script_gameobjectname rainy_platform_collision");

    ents = getEntArray("rainy_care_package_collision", "script_gameobjectname");
    if (isDefined(ents) && ents.size > 0)
        return rainySetPlatformCollisionDonor(ents[0], "script_gameobjectname rainy_care_package_collision");

    ents = getEntArray("rainy_platform_collision", "script_noteworthy");
    if (isDefined(ents) && ents.size > 0)
        return rainySetPlatformCollisionDonor(ents[0], "script_noteworthy rainy_platform_collision");

    return false;
}

rainyTryBrushIndexPlatformCollision()
{
    /*
        Manual/debug fallback.
        Example: set rainy_platform_brush_index 0
    */
    idxDvar = getDvar("rainy_platform_brush_index");
    if (idxDvar == "")
        return false;

    brushIdx = getDvarInt("rainy_platform_brush_index");
    if (brushIdx < 0)
        return false;

    brushes = getEntArray("script_brushmodel", "classname");
    if (isDefined(brushes) && brushes.size > brushIdx)
        return rainySetPlatformCollisionDonor(brushes[brushIdx], "script_brushmodel index #" + brushIdx);

    return false;
}

rainyPlatformGetAnyPlayer()
{
    if (!isDefined(level.players))
        return undefined;
    ...
    for (i = 0; i < level.players.size; i++)
    {
        if (isDefined(level.players[i]) && level.players[i] isHost())
...
rainyPlatformTraceHits(start, end, crate, ignoreEnt)
{
    ...
    if (isDefined(trace["entity"]) && trace["entity"] == crate)
        return true;
...
rainyPlatformCountTestHits(crate, pos, ignoreEnt)
{
    hits = 0;

    if (rainyPlatformTraceHits(pos + (80, 0, 15), pos + (-80, 0, 15), crate, ignoreEnt))
        hits++;
    if (rainyPlatformTraceHits(pos + (0, 80, 15), pos + (0, -80, 15), crate, ignoreEnt))
        hits++;
    if (rainyPlatformTraceHits(pos + (80, 20, 25), pos + (-80, 20, 25), crate, ignoreEnt))
        hits++;
    if (rainyPlatformTraceHits(pos + (80, -20, 25), pos + (-80, -20, 25), crate, ignoreEnt))
        hits++;
    if (rainyPlatformTraceHits(pos + (20, 80, 25), pos + (20, -80, 25), crate, ignoreEnt))
        hits++;
    if (rainyPlatformTraceHits(pos + (-20, 80, 25), pos + (-20, -80, 25), crate, ignoreEnt))
        hits++;
...
rainyPlatformCandidateHasCollision(candidateEnt)
{
    if (!isDefined(candidateEnt))
        return false;

    player = rainyPlatformGetAnyPlayer();
    ...
    {
        ignoreEnt = player;
        testPos = player.origin + (0, 0, 260);
    }
    else if (isDefined(candidateEnt.origin))
    {
        testPos = candidateEnt.origin + (0, 0, 500);
    }
    ...
    hits = rainyPlatformCountTestHits(crate, testPos, ignoreEnt);
    crate delete();
...
rainyTryCandidateArrayForRealCollision(candidates, label)
{
    ...
    for (i = 0; i < candidates.size; i++)
    {
        ...
        if (rainyPlatformCandidateHasCollision(candidates[i]))
            return rainySetPlatformCollisionDonor(candidates[i], label + " #" + i);
    }

    return false;
}

rainyTryKnownAirdropPlatformCollision()
{
    // Do not blindly trust airdrop_pallet on ported maps. mp_luna proved that
    // this tag can exist on dead placeholders, so verify collision before use.
    pallets = getEntArray("airdrop_pallet", "script_gameobjectname");
    if (rainyTryCandidateArrayForRealCollision(pallets, "verified airdrop_pallet"))
        return true;

    candidates = [];
    names = [];
    names[0] = "iw3x_entity_carepackage";
    names[1] = "iw3x_entity_airdroppallet";
    names[2] = "airdrop_crate_collision";
    names[3] = "care_package_collision";
    ...
    for (i = 0; i < names.size; i++)
    {
        ...
        for (j = 0; j < ents.size; j++)
            candidates[candidates.size] = ents[j];
    }

    return rainyTryCandidateArrayForRealCollision(candidates, "verified known airdrop targetname");
}

rainyTryAutoScanBrushPlatformCollision()
{
    /*
        Last-resort generic custom-map fallback.
        This mirrors the debug script's successful rbd_use 0 path, but only
        accepts a brush after it proves cloned collision with trace hits.
    */
    if (getDvarInt("rainy_disable_platform_autoscan") == 1)
        return false;
    ...
    {
        if (rainyPlatformCandidateHasCollision(brushes[i]))
            return rainySetPlatformCollisionDonor(brushes[i], "auto-scanned script_brushmodel #" + i);
    }

    println("^1[Rainy Platforms] No verified platform collision donor found on this map.");
    return false;
}

rainyApplyKnownMapPlatformDonor()
{
    // Per-map known-good platform collision donors, merged in from the old
    // standalone rainy_platform_donors.gsc companion script so this file is
    // fully self-contained. Previously that file and this one were two
    // separate .gsc files each running their own main()/init() at level
    // start with no guaranteed order between them - whichever one's main()
    // happened to fire first is what set level.rainyPlatformCollisionDonor,
    // which is an unnecessary race now removed by calling this directly
    // from rainyEnsureAirdropCrateCollision() below, in this same file.
    // Each entry was confirmed solid using the brush collision debugger
    // (rbd_scan / rbd_spawn - stood on it, verified real collision) before
    // being added here.
    //
    // Guarded so it never overwrites a donor that's already set some other
    // way (a genuine map-baked level.rainyPlatformCollisionDonor from the
    // map's own mapscript, or an earlier call this same level) - matches
    // the "map-baked wins" priority already documented in
    // rainyEnsureAirdropCrateCollision() below.
    if (isDefined(level.rainyPlatformCollisionDonor))
        return;
    ...
    if (mapname == "mp_luna")
    {
        candidate = getEnt("train", "targetname");
    }
    else if (mapname == "mp_luna2")
    {
        brushes = getEntArray("script_brushmodel", "classname");
        if (isDefined(brushes) && brushes.size > 0)
            candidate = brushes[0];
    }
    else if (mapname == "mp_bjwifi_port")
    {
        brushes = getEntArray("script_brushmodel", "classname");
        if (isDefined(brushes) && brushes.size > 1)
            candidate = brushes[1];
    }
    else if (mapname == "mp_nuked")
    {
        brushes = getEntArray("script_brushmodel", "classname");
        if (isDefined(brushes) && brushes.size > 32)
            candidate = brushes[32];
    }
    ...
    else if (mapname == "mp_district")
    {
        brushes = getEntArray("script_brushmodel", "classname");
        if (isDefined(brushes) && brushes.size > 8)
            candidate = brushes[8];
    }
    ...
    else if (mapname == "mp_pval")
    {
        // "mp_pval" is ambiguous: stock MW2 Favela AND at least one custom port
        // both report this same internal mapname (same root cause as the
        // mp_nuked collision noted above). Brush COUNT is what actually tells
        // them apart - the custom port has ~213 script_brushmodel entities,
        // far more than stock Favela ever has, so that's used as the fork here
        // instead of trusting the name string alone.
        brushes = getEntArray("script_brushmodel", "classname");

        if (isDefined(brushes) && brushes.size > 200)
        {
            // Custom port. Candidate index must come from the SAME deduplicated
            // list RBD Cycle Spawn used to find/verify #8 - a fresh raw
            // getEntArray call here is not guaranteed to line up with that
            // list's ordering if any duplicate entities exist on this map.
            rainyRbdBuildCandidates();
            if (isDefined(level.rainyRbdCandidates) && level.rainyRbdCandidates.size > 8)
                candidate = level.rainyRbdCandidates[8];
        }
        else if (isDefined(brushes) && brushes.size > 8)
        {
            // Stock Favela path, unchanged.
            candidate = brushes[8];
        }
    }
    // Add more confirmed maps here, one block each:
    //
    // else if (mapname == "mp_nextmap")
    // {
    //     candidate = getEnt("some_targetname", "targetname");
    // }

    if (!isDefined(candidate))
        return;

    // Verify before committing - same rule rainyTryKnownAirdropPlatformCollision()
    // already applies to airdrop_pallet tags just below: a mapname match is not
    // proof this remembered entity/brush index still points at real collision.
    // If a different map happens to reuse this same mapname string (this is
    // exactly what broke mp_nuked - a CoD4-ported map and a separately-built
    // Nuketown recreation both use that internal name), or the brush ordering
    // shifted between builds/versions of the same port, this stops it from
    // locking in a dead donor and silently blocking every later, working
    // fallback step (explicit .ents alias, existing airDropCrateCollision,
    // verified airdrop_pallet, auto-scan) the way a blind assignment would.
    if (rainyPlatformCandidateHasCollision(candidate))
        level.rainyPlatformCollisionDonor = candidate;
}
...
rainyEnsureAirdropCrateCollision()
{
    /*
        Resolution order:
        0. Known-map donor table: rainyApplyKnownMapPlatformDonor() above -
           fills in level.rainyPlatformCollisionDonor for specific ported/
           custom maps we've already confirmed a donor for, but only if
           nothing has set it already, AND only after re-verifying that
           candidate still gives real collision on this load (guards against
           a different map reusing the same mapname string).
        1. Map-baked donor: level.rainyPlatformCollisionDonor, set directly in
           the map's own mapscript (e.g. maps/mp/mp_luna.gsc) at build time.
           This variable name is unique to this system - nothing else could
           plausibly set it - so it's safe to trust immediately, unlike the
           generic level.airDropCrateCollision case below.
        2. Explicit .ents alias entity: targetname rainy_platform_collision.
        3. Manual debug override: set rainy_platform_brush_index <n>.
        4. Existing stock-map level.airDropCrateCollision.
        5. Verified airdrop_pallet / known care-package candidates.
        6. Auto-scan every script_brushmodel and use the first one that really
           clones standable collision.

        Steps 0-4 are all cheap (a handful of getEnt/getDvar lookups) and
        re-run fresh on every single call, including the manual brush-index
        override, so console-testing "set rainy_platform_brush_index <n>"
        always takes effect immediately even on a map that already failed
        every other step once. Steps 5 and 6 are the expensive ones (they
        spawn+trace+delete a real test crate per candidate) - this function
        gets re-called on every single platform-spawn button press (see the
        "re-resolve right before cloning" comment in spawnSingleCrate/
        spawnPlatformAtCrosshair), so on an unsupported custom map with many
        script_brushmodel entities and no real donor, spamming the spawn
        button used to re-run that full expensive scan every single press.
        level.rainyPlatformCollisionScanFailed caches a genuine "nothing
        worked" result from steps 5/6 so they're skipped on later calls -
        it only ever gets set after both have already been tried and both
        failed, and nothing above short-circuits it, so it can never mask a
        donor that would otherwise have resolved.
    */

    rainyApplyKnownMapPlatformDonor();

    if (isDefined(level.rainyPlatformCollisionDonor))
        return rainySetPlatformCollisionDonor(level.rainyPlatformCollisionDonor, "map-baked level.rainyPlatformCollisionDonor");

    if (rainyTryExplicitPlatformCollision())
        return;

    if (rainyTryBrushIndexPlatformCollision())
        return;

    if (isDefined(level.airDropCrateCollision) && isDefined(level.rainyPlatformCollisionResolved) && level.rainyPlatformCollisionResolved)
        return;

    // Removed a guard that used to trust level.airDropCrateCollision just because
    // it was defined, even if nothing in this resolver ever verified it. Anything
    // that sets that variable outside of rainySetPlatformCollisionDonor() (another
    // script, a leftover value, a stray dvar) could otherwise permanently lock in
    // a dead placeholder and skip the marker/auto-scan checks below entirely.
    // Only OUR OWN verified assignment (rainyPlatformCollisionResolved) is trusted.

    if (isDefined(level.rainyPlatformCollisionScanFailed) && level.rainyPlatformCollisionScanFailed)
        return;

    if (rainyTryKnownAirdropPlatformCollision())
        return;

    if (rainyTryAutoScanBrushPlatformCollision())
        return;

    level.rainyPlatformCollisionScanFailed = true;
}

rainyRbdMakeLabel(ent, source)
{
    // Barebones copy of rainy_brush_collision_debug.gsc's rbd_make_label().
    // Only used for the println() trace line in rainyCycleRbdSpawn below -
    // never shown on-screen, so it costs nothing against the configstring
    // budget regardless of how verbose it is.
    tname = "<none>";
    gameObj = "<none>";
    modelName = "<unknown>";
    ...
    if (isDefined(ent.script_gameobjectname))
        gameObj = ent.script_gameobjectname;
    ...
    return source + " | targetname=" + tname + " | gameobject=" + gameObj + " | model=" + modelName + " | origin=(" + org[0] + ", " + org[1] + ", " + org[2] + ")";
}

rainyRbdAddCandidate(ent, source)
{
    // Barebones copy of rainy_brush_collision_debug.gsc's rbd_add_candidate().
    // Deliberately kept on its own level.rainyRbdCandidates/
    // level.rainyRbdCandidateLabels arrays rather than reusing
    // level.rbd_candidates/level.rbd_candidateLabels - if
    // rainy_brush_collision_debug.gsc is ever loaded alongside this file
    // too (e.g. still using it for map-porting debug work), its own
    // rbd_scan/rbd_test/etc. console commands can rebuild ITS arrays at any
    // time without touching or racing the menu's copy, and vice versa.
    if (!isDefined(ent))
        return;

    if (!isDefined(level.rainyRbdCandidates))
        level.rainyRbdCandidates = [];

    for (i = 0; i < level.rainyRbdCandidates.size; i++)
    {
        if (level.rainyRbdCandidates[i] == ent)
            return;
    }

    idx = level.rainyRbdCandidates.size;
    level.rainyRbdCandidates[idx] = ent;
    level.rainyRbdCandidateLabels[idx] = rainyRbdMakeLabel(ent, source);
}

rainyRbdBuildCandidates()
{
    // Barebones copy of rainy_brush_collision_debug.gsc's rbd_build_candidates() -
    // same scan order, same fallback rules, same known-tag lists, just against
    // this file's own level.rainyRbdCandidates array so RBD Cycle Spawn no
    // longer needs rainy_brush_collision_debug.gsc loaded at all. Everything
    // else in that script (rbd_scan/rbd_test/rbd_spawnall/rbd_try_collider and
    // its console-dvar loop) is debug-only tooling the menu never calls, so
    // none of it needed to come along for this.
    level.rainyRbdCandidates = [];
    level.rainyRbdCandidateLabels = [];

    // Best general grab: every script_brushmodel currently registered on the map.
    brushes = getEntArray("script_brushmodel", "classname");
    if (isDefined(brushes))
    {
        for (i = 0; i < brushes.size; i++)
            rainyRbdAddCandidate(brushes[i], "classname=script_brushmodel");
    }

    // Some porting toolchains (BO2-to-IW4x in particular) may not expose
    // collidable objects as script_brushmodel at all. If the above found
    // nothing, widen the search to every script_model on the map too -
    // capped, since script_model can legitimately number in the hundreds on
    // a detailed map. Only runs automatically when script_brushmodel came up
    // empty; can also be forced with set rbd_scan_models 1 regardless of
    // brush count (same dvar the standalone debug script uses, so existing
    // habits/muscle memory still work even without that script loaded).
    if (level.rainyRbdCandidates.size == 0 || getDvarInt("rbd_scan_models") == 1)
    {
        ...
        if (isDefined(models))
        {
            ...
            for (i = 0; i < limit; i++)
                rainyRbdAddCandidate(models[i], "classname=script_model");

            if (models.size > cap)
                println("^3[RBD Cycle] script_model scan capped at " + cap + " of " + models.size + " found.");
        }
    }

    // Extra grabs for known useful gameobject tags, in case a map/toolchain
    // does not return every brush through the classname query above.
    gameObjNames = [];
    gameObjNames[0] = "airdrop_pallet";
    gameObjNames[1] = "sab";
    gameObjNames[2] = "bombzone";
    gameObjNames[3] = "dd_bombzone";
    gameObjNames[4] = "hq";
    gameObjNames[5] = "raid_coop_clip";

    for (i = 0; i < gameObjNames.size; i++)
    {
        arr = getEntArray(gameObjNames[i], "script_gameobjectname");
        if (!isDefined(arr))
            continue;

        for (j = 0; j < arr.size; j++)
            rainyRbdAddCandidate(arr[j], "script_gameobjectname=" + gameObjNames[i]);
    }

    // Known targetnames from MW2/IW4x/ported-map care-package collision work.
    fixedNames = [];
    fixedNames[0] = "rainy_platform_collision";
    fixedNames[1] = "iw3x_entity_carepackage";
    fixedNames[2] = "iw3x_entity_airdroppallet";
    fixedNames[3] = "airdrop_crate_collision";
    fixedNames[4] = "care_package_collision";
    fixedNames[5] = "train";
    ...
    for (i = 0; i < fixedNames.size; i++)
    {
        ent = getEnt(fixedNames[i], "targetname");
        if (isDefined(ent))
            rainyRbdAddCandidate(ent, "targetname=" + fixedNames[i]);
    }
}

/*
    Admin Menu - Print Messages: master on/off switch for every piece of text
    the mod menu itself prints on screen. Gated at the two functions every
    such message ultimately funnels through - rainyShowRaisedMessage (menu
    action confirmations/feedback toasts) and rainyAddBroadcastFeedEntry
    (Trickshot Distance, Almost Hits, and Hitmarker shot-feed entries) - so
    this one switch covers both without needing to touch every call site.
    The welcome banner is a fixed one-time connect banner, not a "message"
    in this sense, and is intentionally left alone (matches the file's own
    existing distinction between the two - see rainyShowWelcomeBanner's
    surrounding comments).
*/
rainyTogglePrintMessages()
{
    if (!isDefined(level.rainyPrintMessagesEnabled))
        level.rainyPrintMessagesEnabled = true;
    if (level.rainyPrintMessagesEnabled)
    {
        // Show the OFF confirmation BEFORE flipping the flag - otherwise
        // this exact message would suppress itself and the host would get
        // no feedback that the toggle actually worked.
        self thread rainyShowRaisedMessage("^5Print Messages ^7[^5OFF^7]");
        level.rainyPrintMessagesEnabled = false;
    }
    else
    {
        level.rainyPrintMessagesEnabled = true;
        self thread rainyShowRaisedMessage("^5Print Messages ^7[^5ON^7]");
    }
}
rainyCycleRbdSpawn()
{
    /*
        Menu-driven version of the debug script's rbd_spawn workflow -
        one button press per candidate instead of typing console commands
        by hand. Fully self-contained as of this version - the candidate
        scan (rainyRbdBuildCandidates above) is a barebones copy of the
        relevant part of rainy_brush_collision_debug.gsc, so this no longer
        requires that script to be loaded at all. That companion script can
        still be kept around separately for its own console-driven debug
        tools (rbd_scan/rbd_test/rbd_spawnall/rbd_try_collider, etc.) when
        porting a new map, but this menu button no longer depends on it.

        Wraps at however many candidates actually exist on the current map
        (level.rainyRbdCandidates.size) rather than a fixed number - a map
        with 8 candidates wraps at 8, a map with 100 wraps at 100, no
        per-map configuration needed here.

        Configstring budget note: the Admin Menu's "RBD Cycle Spawn [x/y]"
        label (rendered in updateMenuHud) already calls set_text() with a
        different string every time idx changes, which permanently mints one
        configstring per unique index the FIRST time that index is ever shown
        - bounded by level.rainyRbdCandidates.size, but real cost on maps
        with a lot of candidates. The old version ALSO fired an iPrintLnBold
        and a rainyShowRaisedMessage (itself a set_text call) on every single
        press, each baking in the same idx and therefore minting a second
        full set of unique configstrings on top of the menu label's. That's
        why this version shows no on-screen confirmation at all on a normal
        press: the counter you need is already live in the menu label, and
        println() below is free (console/overlay text only, never a
        configstring) so it's kept for debug tracing. The two failure paths
        below are rare (not once-per-press) and use static, non-idx text, so
        a single rainyShowRaisedMessage call for each is effectively free
        against the budget - no need for a duplicate iPrintLnBold too.
    */
    rainyRbdBuildCandidates();

    if (!isDefined(level.rainyRbdCandidates) || level.rainyRbdCandidates.size == 0)
    {
        self thread rainyShowRaisedMessage("^1RBD Cycle Spawn: no candidates");
        return;
    }

    if (!isDefined(self.rainyRbdCycleIndex))
        self.rainyRbdCycleIndex = -1;

    self.rainyRbdCycleIndex++;
    if (self.rainyRbdCycleIndex >= level.rainyRbdCandidates.size)
        self.rainyRbdCycleIndex = 0;

    idx = self.rainyRbdCycleIndex;
    candidateEnt = level.rainyRbdCandidates[idx];
    ...
    if (!isDefined(crate))
    {
        self thread rainyShowRaisedMessage("^1RBD Cycle Spawn: spawn failed");
        return;
    }
    ...
    rainyTrackSpawnedCrate(crate);

    label = "<no label>";
    if (isDefined(level.rainyRbdCandidateLabels) && level.rainyRbdCandidateLabels.size > idx)
        label = level.rainyRbdCandidateLabels[idx];

    // No on-screen confirmation on success by design (see budget note above) -
    // the live counter in the Admin Menu label is the feedback for this action.
    println("^3[RBD Cycle] Spawned candidate #" + idx + " -> " + label);
}
...
spawnPlatformAtCrosshair()
{
    ...
    crate setModel("com_plasticcase_friendly");
    // Re-resolve right before cloning. Custom maps can define/overwrite the
    // donor during map init; this keeps Luna using the known-good debug donor.
    rainyEnsureAirdropCrateCollision();
...
rainyTrackSpawnedCrate(crate)
{
    // Shared append-with-cap for every spawned platform/RBD-test crate, used
    // by spawnPlatformAtCrosshair/spawnSingleCrate/rainyCycleRbdSpawn instead
    // of each pushing into level.rainySpawnedCrates directly. Without a cap,
    // a player who keeps spawning platforms without ever running Remove All
    // Platforms grows both this array and the live entity count in the world
    // without bound. Once the cap is hit, this deletes the OLDEST tracked
    // crate and recycles its slot rather than refusing new spawns outright -
    // the feature keeps working exactly the same from the player's side, it
    // just can't accumulate forever underneath them.
    if (!isDefined(level.rainySpawnedCrates))
        level.rainySpawnedCrates = [];

    cap = 64;
    if (level.rainySpawnedCrates.size >= cap)
    {
        oldest = level.rainySpawnedCrates[0];
        if (isDefined(oldest))
            oldest delete();
        for (i = 0; i < level.rainySpawnedCrates.size - 1; i++)
            level.rainySpawnedCrates[i] = level.rainySpawnedCrates[i + 1];
        level.rainySpawnedCrates[level.rainySpawnedCrates.size - 1] = crate;
    }
    else
    {
...
rainySetWallbangDvars(enabled)
{
    if (enabled)
    {
        ...
        setDvar("bg_bulletExplDmgFactor", "100"); // domain is 0-100 - 9999 was silently clamped every call, spamming rejection warnings for nothing
...
/*
    Restart/end actions are launched in their own worker threads.

    The menu controls thread has level endon("rainy_shutdown"). If that same
    thread calls level notify("rainy_shutdown") synchronously, it kills itself
    immediately at the notify and never reaches map_restart()/exitLevel(). That
    leaves the message on screen and the menu controls dead. A separate worker
    does not inherit the menu thread's endon, so it can finish the transition.
*/
rainyRestartGame()
{
    self closeMenuHud();
    self thread rainyRestartGameWorker();
}
rainyRestartGameWorker()
{
    self endon("disconnect");
    ...
    // Notify BEFORE teardown, matching the only pattern in this file already proven
    // safe (trickshotGlobalEndGameMonitor's F3 path, rainyCleanupOnGameEnd). Every
    // other level endon("rainy_shutdown") watcher loop across every player needs to
    // actually stop before we start destroying HUD elements and unlinking entities
    // underneath them - swapping this order lets those loops keep running mid-teardown.
    level.rainyLevelShuttingDown = true;
    level notify("rainy_shutdown");

    // Full per-player HUD teardown, not just the host - mirrors
    // trickshotGlobalEndGameMonitor's own loop and its stated reason: the notify above
    // only stops loops from building new HUD elements, it doesn't destroy whatever a
    // given player already has on screen. Bots are excluded since they have no HUD.
    if (isDefined(level.players))
    {
        for (i = 0; i < level.players.size; i++)
        {
...
{
    self closeMenuHud();
    self thread rainyInstantEndGameWorker();
}
rainyInstantEndGameWorker()
{
    self endon("disconnect");
    ...
    // Same notify-before-teardown ordering and per-player teardown as
    // rainyRestartGameWorker() above, for the same reasons.
    level.rainyLevelShuttingDown = true;
    level notify("rainy_shutdown");
